...
$Operatori = $dbh->getOperatoriAttivi();
...
$Vendite = $dbh->getVendite($idCicloProduttivo);
$Magazzini = $dbh->getMagazziniListComplete();
$Raccolti= $dbh->getRaccolti($idCicloProduttivo);
?>
...
<div class="sezione">
        ...
        <?php if (!isset($ultima_lavorazione['data_fine'])) { ?>
                ...
                <div class="input-line">
                    ...
                    <div class="input-group">
                        <div class="lavorazione-pulsanti">
                            <label for="concludiLavorazioneData" class="hidden">Fine lavorazione</label>
                            <input type="submit" id="concludi" class="concludi-lavorazione orange-on-white" value="Concludi" />
                        </div>
                    </div>
                </div>
            </form>
            <form id="newTurnoLavorativo">
                ...
                <div class="input-group">
                    ...
                    <select id="attrezzi" class="select-multiple" name="attrezzi[]" multiple>
                ...
                <div class="input-group">
                    ...
                    <input id="prodottiQt" type="number" min="1" step="1" />
                </div>
    ...
    <? if (count($Vendite) != 0): ?>
        <h3>Tutte le Vendite</h3>
        <ul>
            <?php foreach ($Vendite as $vendita): ?>
                In data <?= htmlspecialchars($vendita["data_vendita"]) ?> venduto il raccolto del
                <?= htmlspecialchars($vendita["data_raccolta"]) ?> a <?= htmlspecialchars($vendita["acquirente"]) ?> per
                <?= htmlspecialchars($vendita["ricavo"]) ?> €
            <?php endforeach ?>
        </ul>
    <? endif; ?>

    <? if (count($Raccolti) != 0): ?>
        <h3>Tutte i raccolti</h3>
        <ul>
            <?php foreach ($Raccolti as $raccolto): ?>
                In data <?= htmlspecialchars($raccolto["data"]) ?> raccolti
                <?= htmlspecialchars($raccolto["quantita"]) ?> depositati nel silo <?= htmlspecialchars($raccolto["idSilo"]) ?> 
            <?php endforeach ?>
        </ul>
    <? endif; ?>
